...
function Warn(props) {
    const listTab = [
        { id: 'all', name: 'Tất cả' }, { id: 'open', name: 'Đang lỗi' }, { id: 'closed', name: 'Đã khắc phục' }
    ]
        ...
        <>
            <div className='DAT_WarnHeader'>
                <div className='DAT_WarnHeader_Title'>
                    <LuMailWarning color='gray' size={25} /><span>Cảnh báo</span>
                </div>
            ...
            <div className='DAT_Warn'>
                ...
                {isMobile.value
                    ...
                    ? <div className="DAT_Toollist_Tab_Mobile">
                        <button className="DAT_Toollist_Tab_Mobile_content" onClick={() => tabMobile.value = !tabMobile.value} > <span> {tabLable.value}</span>  {(tabMobile.value) ? <IoIosArrowDown /> : <IoIosArrowRoundForward />} </button>
                        <div className="DAT_Toollist_Tab_Mobile_list" >
                            {listTab.map((item, i) => {

                                return (
                                    <div className="DAT_Toollist_Tab_Mobile_list_item" style={{ display: (tabMobile.value) ? "block" : "none" }} key={i} id={item.id} onClick={(e) => handleTabMobile(e)} >{i + 1}: {item.name}</div>
                                )
                            ...
                            return (
                                ...
                                (tab.value === item.id)
                                    ? <div key={i} className="DAT_Toollist_Tab_main">
                                        ...
                                        <span className="DAT_Toollist_Tab_main_content1" id={item.id} style={{ backgroundColor: "White", color: "black", borderRadius: "10px 10px 0 0" }} onClick={(e) => tab.value = item.id}>{item.name}</span>
                                        <p className="DAT_Toollist_Tab_main_right"></p>
                                    </div>
                                    : <span className="DAT_Toollist_Tab_main_content2" key={i} id={item.id} style={{ backgroundColor: "#dadada" }} onClick={(e) => tab.value = item.id}>{item.name}</span>
